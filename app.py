...
if st.button("Calcular"):
    try:
        ...
        # Correção final dos termos
        st.header("Previsões de Saída")
        st.markdown(f"""
        - Mínimo: **{hora_saida_5h.strftime('%H:%M')}** ({minutos_intervalo_5h:.0f}min de intervalo)
        - Zerado: **{hora_saida_8h.strftime('%H:%M')}** ({minutos_intervalo_demais:.0f}min de almoço)
        - Máximo: **{hora_saida_10h.strftime('%H:%M')}** ({minutos_intervalo_demais:.0f}min de almoço)
        """)
